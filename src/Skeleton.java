public class Skeleton extends Node {
	...
	static final void drawEntities(int var0, int var1, int var2, int var3) {
		...
		if (Projectile.localPlayer.x >> 7 == Client.destinationX
				&& Projectile.localPlayer.y >> 7 == Client.destinationY) {
		...
		int var6;
		for (Projectile var4 = (Projectile) Client.projectiles
				.last(); var4 != null; var4 = (Projectile) Client.projectiles.previous()) {
				...
				if (Client.cycle >= var4.cycleStart) {
					if (var4.targetIndex > 0) {
						...
						if (var23 != null && var23.x >= 0 && var23.x < 13312 && var23.y >= 0 && var23.y < 13312) {
							var4.setDestination(var23.x, var23.y,
									Renderable.getTileHeight(var23.x, var23.y, var4.plane) - var4.endHeight,
									Client.cycle);
						}
					}
					...
					if (var4.targetIndex < 0) {
						...
						if (var25 != null && var25.x >= 0 && var25.x < 13312 && var25.y >= 0 && var25.y < 13312) {
							var4.setDestination(var25.x, var25.y,
									Renderable.getTileHeight(var25.x, var25.y, var4.plane) - var4.endHeight,
									Client.cycle);
						}
					}

					var4.advance(Client.graphicsCycle);
					class36.scene.drawEntity(Clock.Client_plane, (int) var4.x, (int) var4.y, (int) var4.z, 60, var4,
							var4.yaw, -1L, false);
				}
			} else {
				var4.remove();
		...
		for (GraphicsObject var24 = (GraphicsObject) Client.graphicsObjects
				.last(); var24 != null; var24 = (GraphicsObject) Client.graphicsObjects.previous()) {
		...
		if (Client.oculusOrbState == 1 && Client.staffModLevel >= 2 && Client.cycle % 50 == 0
				&& (ModeWhere.oculusOrbFocalPointX >> 7 != Projectile.localPlayer.x >> 7
						|| GrandExchangeEvents.oculusOrbFocalPointY >> 7 != Projectile.localPlayer.y >> 7)) {
			...
			PacketBufferNode var19 = ObjectComposition.getPacketBufferNode(ClientPacket.field3182,
		...
		if (!Client.isCameraLocked) {
			...
			if (class10.clientPreferences.method2434()) {
				...
				if (var12 - class152.cameraY < 800
						&& (Tiles.Tiles_renderFlags[Clock.Client_plane][class208.cameraX >> 7][ByteArrayPool.cameraZ >> 7]
								& 4) != 0) {
					var11 = Clock.Client_plane;
				} else {
					var11 = 3;
		...
		for (var16 = 0; var16 < 5; ++var16) {
			if (Client.field771[var16]) {
				var17 = (int) (Math.random() * (double) (Client.field772[var16] * 2 + 1)
						- (double) Client.field772[var16]
						+ Math.sin((double) Client.field763[var16] * ((double) Client.field774[var16] / 100.0))
								* (double) Client.field695[var16]);
				if (var16 == 0) {
		...
		class36.scene.draw(class208.cameraX, class152.cameraY, ByteArrayPool.cameraZ, MusicPatchNode.cameraPitch,
				class291.cameraYaw, var10);
		...
		if (Client.hintArrowType == 2) {
			Decimator.worldToScreen(Client.hintArrowSubX * 64 + (Client.hintArrowX - class213.baseX * 64 << 7),
					Client.hintArrowSubY * 64 + (Client.hintArrowY - class101.baseY * 64 << 7),
					Client.hintArrowHeight * 4);
			if (Client.viewportTempX > -1 && Client.cycle % 20 < 10) {
				ReflectionCheck.headIconHintSprites[0].drawTransBgAt(var0 + Client.viewportTempX - 12,
		...
		if (Client.isLoading) {
			int var20 = 0;
			var20 = var20 + NetCache.NetCache_pendingPriorityResponsesCount
